...
const Banner = () => {
  return (
    <section className='banner w-full h-screen overflow-hidden bg-black grid grid-cols-1 md:grid-cols-2 justify-between items-center overflow-x-hidden'>
      <div className="banner-content-main grid grid-cols-1 md:grid-cols-2 items-center justify-center">
        ...
        <button className='btn text-black bg-white px-14 rounded-sm hover:text-white'>Get Start</button>
        </div>
        <div className="banner-bg hidden md:block">
          <img className='h-full w-full ml-[100px]' src={bg} alt="" />
        </div>
      </div>
      <div className="banner-image mt-14 md:mt-0">
        <img src={car} alt="" />
      </div>
    </section>
  );
